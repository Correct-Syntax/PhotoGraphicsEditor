...
class OpenFile_Dialog(object):
    """ The base class for the open file dialog. """
    def __init__(self, master, title=None, filetypes=['.png', '.jpg']):
        self.master = master
        self.directory = None
        ...
        ## Preview frame
        ThumbnailPreview_lblfrm = Frame(self.sidebarframe, relief=SOLID, bd=1, bg='grey40')
        ThumbnailPreview_lblfrm.pack(padx=4, pady=4, side=TOP)

        self.ThumbnailPreview_Canvas = Canvas(ThumbnailPreview_lblfrm,
                                              highlightthickness=0, width=140,
                                              height=140, bg="grey40")
        self.ThumbnailPreview_Canvas.pack(padx=4, pady=4, side=TOP)
        ...
        ToolTip(self.ok_button, "Open the selected file", wtype='FLATBUTTON')
    ...
    def go(self, directory_or_file="C:/", pattern="*", default=""):
    ...
    def generate_preview(self):
        imgfile = Image.open(self.get_selection())
        
        ## Resize the image to the correct dimensions
        if imgfile.size[0] == imgfile.size[1]:
            self.prev_image = imgfile.resize((140, 140))

        elif imgfile.size[0] > imgfile.size[1]:
            self.prev_image = imgfile.resize((140, int(round(float(imgfile.size[1]) * 140/imgfile.size[0]))))

        else:
            self.prev_image = imgfile.resize((int(round(float(imgfile.size[0]) * 140/imgfile.size[1])), 140))

        ## Create the preview
        self.finalprev_image = ImageTk.PhotoImage(self.prev_image)
        self.ThumbnailPreview_Canvas.create_image(0, 0, anchor=NW, image=self.finalprev_image)
    
    def cancel_command(self, event=None):
        self.quit()


class OpenFileDialog(OpenFile_Dialog):
    """File selection dialog to open a file. """
